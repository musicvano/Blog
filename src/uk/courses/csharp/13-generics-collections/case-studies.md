...
string[] cities = ["Мадрид", "Брюссель", "Париж", "Відень"];
DateOnly[] deadlines =
[
...
Для кожного виклику компілятор виводить аргумент типу з типу масиву: `int`, `string`, `DateOnly`. У виклику `FindMax<double>([2.5, 7])` тип указано явно, тому ціле 7 перетворюється на `double`. Рядки порівнюються з урахуванням культури, тому «Париж» стоїть після «Мадрида» за українською абеткою. Результат:

```
Найбільший бал: 95
Останнє місто за абеткою: Париж
